...
/**
 * وظيفة زر التحديث في نظام الاستثمار المتكامل
 * يقوم بتحديث واجهة المستخدم وإعادة تحميل البيانات عند تجمد البرنامج
 */
...
function addRefreshButton() {
    ...
    // التحقق من عدم وجود زر التحديث مسبقًا
    ...
    // إضافة الزر إلى منطقة السحب في شريط العنوان (قبل عناصر التحكم في النافذة)
  ...
  function refreshApplication() {
    ...
    // تغيير حالة الزر لإظهار الدوران
    ...
    try {
      ...
      // 4. تحديث الإشعارات والتنبيهات إن وجدت
  ...
  // إضافة وظيفة مساعدة لإعادة تحميل البيانات من البداية (لحالات التجمد الشديد)
  function hardRefresh() {
    // محاولة إعادة تهيئة التطبيق بالكامل
    try {
      // تحميل البيانات من التخزين المحلي
      if (window.loadData) {
        window.loadData();
      ...
      // إعادة تهيئة مستمعي الأحداث
      if (window.initEventListeners) {
        window.initEventListeners();
      }
      
      // إعادة تعيين التاريخ الحالي كافتراضي
      if (window.setCurrentDateAsDefault) {
        window.setCurrentDateAsDefault();
      }
      ...
      // تفعيل الزر العائم
      if (window.initFloatingActionButton) {
        window.initFloatingActionButton();
      }
      ...
      // عرض إشعار بنجاح إعادة التحميل
      if (window.showNotification) {
        window.showNotification('تم إعادة تحميل التطبيق بالكامل بنجاح', 'success');
      }
      
      return true;
    } catch (error) {
      console.error('خطأ في إعادة تحميل التطبيق بالكامل:', error);
      
      // عرض إشعار بالخطأ
      if (window.showNotification) {
        window.showNotification('حدث خطأ أثناء إعادة تحميل التطبيق', 'error');
      }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // إضافة اختصار لوحة المفاتيح F5 لتحديث التطبيق
    document.addEventListener('keydown', function(event) {
      // عند الضغط على F5
      if (event.key === 'F5') {
        // منع السلوك الافتراضي (تحديث الصفحة بالكامل)
        event.preventDefault();
        
        // استدعاء وظيفة تحديث التطبيق
        refreshApplication();
      }
      
      // عند الضغط على Ctrl+F5 (إعادة تحميل كاملة)
      if (event.key === 'F5' && (event.ctrlKey || event.metaKey)) {
        // منع السلوك الافتراضي
        event.preventDefault();
        
        // استدعاء وظيفة إعادة التحميل الكاملة
        hardRefresh();
      }
    });
    
    console.log('تم تهيئة زر التحديث واختصارات لوحة المفاتيح');
  });
